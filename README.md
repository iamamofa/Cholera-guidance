# Vibrio cholerae Genomic Surveillance
## A PHA4GE Best Practices Guidance Document

![PHA4GE Logo](phage.png)

**Version:** 1.0 | **Published:** March 2026 | **License:** [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/)

> **Working Group:** PHA4GE Cholera Genomics Working Group  
> **How to cite:** PHA4GE Cholera Genomics Working Group (2026). *Vibrio cholerae Genomic Surveillance: A PHA4GE Best Practices Guidance Document*. https://pha4ge.org/cholera-guidance

---

## Overview

According to the World Health Organisation (WHO), outbreaks of acute watery diarrhea (AWD)-related diseases are likely to occur unless surveillance systems are in place to rapidly detect their associated pathogens and respond accordingly. These materials provide a unified, evidence-based framework for the genomic analysis of *Vibrio cholerae*, covering the full workflow from raw sequencing data to actionable public health outputs.

Cholera remains one of the most significant infectious disease threats globally, with the seventh pandemic still ongoing. Genomic surveillance offers unparalleled resolution for understanding transmission chains, detecting emerging antimicrobial resistance (AMR), and monitoring pandemic lineage evolution in near-real time.

---

## Guidance Documents

| # | Page | Description |
|---|------|-------------|
| 01 | [Introduction & Scope](docs/01-introduction.md) | Purpose, target audience, scope and how to use this document |
| 02 | [Biology of *Vibrio cholerae*](docs/02-biology.md) | Taxonomy, morphology, virulence mechanisms, ecology |
| 03 | [Epidemiology of Cholera](docs/03-epidemiology.md) | Global burden, geographic distribution, transmission dynamics |
| 04 | [Genomics of *V. cholerae*](docs/04-genomics.md) | Genome architecture, key genomic elements, pandemic lineage |
| 05 | [Why Bioinformatics Is Essential](docs/05-why-bioinformatics.md) | Limitations of traditional methods, what WGS enables |
| 06 | [Why PHA4GE Needs to Lead](docs/06-pha4ge-standards.md) | Standards, best practices, community coordination |
| 07 | [Laboratory Prerequisites & Sequencing](docs/07-laboratory.md) | Biosafety, isolation, DNA extraction, sequencing platforms |
| 08 | [Data Management & File Formats](docs/08-data-management.md) | File formats, naming, version control, storage |
| 09 | [Sequence Quality Control](docs/09-quality-control.md) | FastQC, trimming, host depletion, contamination screening |
| 10 | [Genome Assembly](docs/10-assembly.md) | De novo, reference-based, hybrid assembly workflows |
| 11 | [Assembly Quality Assessment](docs/11-assembly-qc.md) | QUAST, CheckM, quality tiers, pass/fail criteria |
| 12 | [Strain Typing & Lineage Classification](docs/12-typing.md) | MLST, cgMLST, pandemic wave assignment, Vibriowatch |
| 13 | [Phylogenetic Analysis](docs/13-phylogenetics.md) | SNP calling, Gubbins, IQ-TREE2, Microreact, molecular clocks |
| 14 | [Antimicrobial Resistance Detection](docs/14-amr.md) | AMR genes, point mutations, quality thresholds |
| 15 | [Virulence Gene Prediction](docs/15-virulence.md) | ctxB allele typing, TCP, VPI islands, VirulenceFinder |
| 16 | [Plasmid Detection & Analysis](docs/16-plasmids.md) | MOB-suite, PlasmidFinder, Chr2 pitfalls |
| 17 | [Interpretation of Results](docs/17-interpretation.md) | Integration framework, SNP thresholds, common pitfalls |
| 18 | [Data Sharing & FAIR Principles](docs/18-data-sharing.md) | Repositories, metadata standards, data sovereignty |
| 19 | [Ethics, Governance & Responsible Use](docs/19-ethics.md) | Patient privacy, equitable collaboration, DURC |
| 20 | [Reporting & Communication](docs/20-reporting.md) | Rapid reports, full reports, IHR obligations |

---

## Case Studies

- **Haiti 2022** — Resurgence of cholera after apparent elimination; imported wave 3 strain
- **Zimbabwe 2018** — Multi-drug resistant O1 El Tor outbreak with SXT ICE characterisation
- **Yemen 2016–present** — Largest recorded cholera outbreak in history; conflict-driven transmission

---

## Contributing

This document is a **living resource**. Contributions are welcomed from the global public health and genomics communities.

1. Open a [GitHub Issue](https://github.com/pha4ge/cholera-guidance/issues) to propose changes
2. Submit a [Pull Request](https://github.com/pha4ge/cholera-guidance/pulls) with modifications
3. Join the PHA4GE Cholera Genomics Working Group at [pha4ge.org](https://pha4ge.org)

---

## Quick Links

- [PHA4GE](https://pha4ge.org)
- [Vibriowatch / PathogenWatch](https://pathogen.watch/collections/vibriowatch)
- [WHO Cholera Fact Sheet](https://www.who.int/news-room/fact-sheets/detail/cholera)
- [GTFCC Roadmap to 2030](https://www.gtfcc.org)
- [NCBI SRA](https://www.ncbi.nlm.nih.gov/sra)
- [CholeraBook](https://vibriowatch.readthedocs.io)

---

*Licensed [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/) · Built by the PHA4GE Cholera Genomics Working Group*